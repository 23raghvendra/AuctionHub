...
export const addNewAuctionItem = catchAsyncErrors(async (req, res, next) => {
  ...
  // Allow start time to be up to 30 seconds in the past to account for processing time
  const now = Date.now();
  const startTimeMs = new Date(startTime).getTime();
  if (startTimeMs < now - 30000) { // 30 seconds grace period
    return next(
      new ErrorHandler(
  ...
  // Removed restriction: Auctioneers can now create multiple active auctions
  try {
    ...
    try {
      ...
      return res.status(201).json({
        success: true,
        message: `Auction item created (local storage) and will be listed on auction page at ${startTime}`,
        auctionItem,
      });

    } catch (localError) {
      console.error("Local save error:", localError);
      return next(new ErrorHandler("Failed to upload image locally after Cloudinary failure.", 500));
    }
  }
});
